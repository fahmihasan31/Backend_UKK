...
const EditMenuModal = ({ isOpen, onClose, item }) => {
  if (!item) {
    return null
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:8000/menu/update/${item.id_menu}`, data, config);
      resetForm();
      onClose();
    } catch (error) {
      setErrorMessage(error.response?.data?.message || 'Error updating menu.');
